...
import { RemovePercentageSlider } from './RemovePercentageSlider';
import { LPTokenDisplay } from './LPTokenDisplay';
 ...
 * - State management via useRemoveLiquidity hook
 */
export const RemoveLiquidityCard: React.FC = () => {
  const {
    formData,
    handlePoolSelect,
  ...
  return (
    <Box sx={{ maxWidth: 600, margin: '0 auto' }}>
      ...
      >
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, fontSize: '0.875rem' }}>
              Redeem your LP tokens for underlying assets
            </Typography>
          </Box>
        ...
        {/* LP token display */}
        {formData.pool && formData.percentage > 0 && (
          <Box sx={{ mb: 3 }}>
            <LPTokenDisplay
              lpBalance={formData.lpBalance}
              lpBalanceFormatted={formData.lpBalanceFormatted}
              lpTokens={formData.lpTokens}
              lpTokensFormatted={formData.lpTokensFormatted}
              poolName={formData.pool.name}
              percentage={formData.percentage}
            />
          </Box>
        )}
        ...
        >
          Removing liquidity will return your tokens proportionally to your pool share
        </Typography>
      </Card>
    </Box>
  );
